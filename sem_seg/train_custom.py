...
parser.add_argument('--grid_size', type=int, default=10, help='Size of grid option: 1,2,5,10 [default: 1]')
...
os.system('cp model_custom.py %s' % (LOG_DIR)) # bkp of model def
...
def load_data_memory(data_dir, file, datasplit_path):
    ...
    df = df.set_index(hash_col, drop=False)
    #df.reset_index(inplace=True)
    df.index.name = 'index'
    df = df.groupby(hash_col).filter(lambda x: len(x) > 100)
    train_hashes, validation_hashes, test_hashes = traintest_split(df, mode='random', path=datasplit_path)
...
def generator(df, hashes, BATCH_SIZE, NUM_POINT, N_AUGMENTATIONS, shuffled=True):
    ...
    for batch in batches:
        ...
        label = np.stack([l.raw_classification.values for l in df_batch])-1
